...
int main()
{
	...
	while(true){
		...
		if(!cin){ // stdin closed without an explicit exit/leave command
			break;
		}
		...
		if(command[0] == "create_ring"){ // createring
			...
			exit(0); // avoid destroying the stack-allocated currentnode while detached threads still hold a pointer to it
